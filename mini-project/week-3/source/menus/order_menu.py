...
#Orders menu interface: handles orders menu navigation and user actions.
def show_orders_menu():
    while True:
        ...
        if choice == "0":
            ...
            crud.print_orders(model.original_orders)
            input()

        elif choice == "2":
            #Get pre-defined customer attributes structure from model module.
            customer_attributes=dict.fromkeys(model.customer_attributes.keys(),'')
            
            #Take inputs for customer attributes and validates the value.
            customer_attributes=crud.get_valid_input_for_order_attributes(original_attributes=customer_attributes, allow_blank=False)

            #Create new item in order list (dict) with customer attributes and default order status.    
            ...
            #Print current orders list and asks for valid order index. 
            crud.print_orders(orders)
            order_index=get_limited_digit_input(limit=len(orders), input_message="Enter an index for order you would like to UPDATE: ")

            #Print status order list and asks for valid order status index.
            ...
            #Save old order status value to show in message after completition. 
            old_status=orders[order_index].get('order_status')

            #Update order status in current order list.
            ...
            #Get value(dict) for choosen order.
            order=orders[order_index]

            #Get valid order attributes and updates chosen order.
            ...
            #Get value(dict) for choosen order to show later in message.
            order=orders[order_index]

            #Confirm deletion and delete chosen order.
